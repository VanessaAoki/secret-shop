  ...
  def show_article(category)
    ...
    category.articles.each do |article|
      article.categories.each do |category|
        out += "<article class=\"columns column is-12 categories-show p-0 m-0\">"
        out += "<div class=\"column is-5 image-container p-0\">"
        out += image_tag article.image, class: 'categories-show-image' if article.image.attached?
        out += '</div>'
        out += '<div class="column is-7 p-4 article-news has-background-white has-text-main is-flex is-flex-direction-column is-justify-content-space-around">'
